...
> 一个从零训练的轻量 68 点人脸关键点模型（自研 LMNet 架构，11.5–15.5 M 参数），INT8 量化后 **15.48 MB**。
> 在 300W 测试集上 NME **5.94%**，相对 24.79 MB 的 FAN4 INT8 教师模型 **改善约 38%**。

中文（默认） | [README.en.md](README.en.md)

![hero](docs/images/hero.png)

*三个样例叠加：真值（绿）vs FAN4 INT8 教师（红）vs 我们的 INT8 LMNet（蓝）。蓝点几乎完全覆盖绿点，红点显著偏离。*
...
| 模型 | 参数量 | 磁盘大小（INT8） | NME | acc@0.05 | acc@0.08 | acc@0.10 |
|---|---|---|---|---|---|---|
| 平均脸基线 | – | – | 0.298 | – | – | – |
| 预训练 face\_alignment 2DFAN4（未微调）| 23.8 M | 90.9 MB FP32 | 0.113 | – | 34.2% | – |
...
完整训练历程、消融实验和失败记录见 [REPORT.md](REPORT.md) 和 [PAPER.md](PAPER.md)。

---

## 项目背景

300W 标准训练集只有约 600 张高质量手动标注图片，对从零训练具有竞争力的关键点模型来说远远不够。本项目通过教师蒸馏 + 大规模伪标签 + 加权采样 + 两阶段微调的方式，在不使用任何预训练权重的前提下，把一个 15.5 M 的轻量自研模型推到了上述精度。

**核心配方**：

1. 在 300W 上把 face\_alignment 2DFAN4 教师微调到 NME 0.0802。
...
最佳 INT8 模型在 `runs/300w_lmnet_w26_100k_finetune/model_int8.onnx`（15.48 MB）。
...
# 输入：(N, 3, 256, 256) float32，[-1, 1] 归一化，crop 自人脸 bbox（建议 1.35× scale）
# 输出：(N, 68, 2) 归一化坐标 [0, 1]
```
...
│   ├── train.py         # YAML 驱动训练器：EMA、AMP、OneCycle/Cosine、可选在线蒸馏
...
├── runs/
│   └── 300w_lmnet_w26_100k_finetune/    # 最佳模型产物
│       ├── best.pt
│       ├── model_int8.onnx (15.48 MB)
│       ├── history.csv
│       └── summary.json
├── REPORT.md / REPORT.en.md     # 详尽实验日志
├── PAPER.md / PAPER.en.md       # 论文风格技术报告
├── optimization_log.md
...
3. **两阶段优于单阶段**：单阶段 Stage A 能到 NME ~0.075；加载 best.pt + 小学习率 cosine + 提高 real\_ratio，单 epoch 即跳到 **0.0514**。
...
6. **face\_alignment 2DFAN4 有几个隐藏契约** —— 不修复这三点会得到 NME 0.91 而非 0.115：
